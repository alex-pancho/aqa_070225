...
def test_insert_and_select(db_connection):
    ...
    cursor.execute("INSERT INTO users (name) VALUES (%s)", ('Gendalf Grey',))
    db_connection.commit()

    cursor.execute("SELECT name FROM users WHERE name = %s", ('Gendalf Grey',))
    result = cursor.fetchone()

    assert result is not None
    assert result[0] == 'Gendalf Grey'

def test_update(db_connection):
    """Test: Update user name."""
    cursor = db_connection.cursor()
    cursor.execute("UPDATE users SET name = %s WHERE name = %s", ('Bilbo', 'Gendalf Gray'))
    db_connection.commit()
