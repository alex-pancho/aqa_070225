...
# Making list from dict by list comprehension and filter all cars later by indexes
filtered_cars = [ 
...
# Sorting results after filtering 
sorted_cars = sorted(filtered_cars, key=lambda x: x[1][4])

#Show only firts 5 sorted results with al criterias 
for car, data in sorted_cars[:5]:
    print(f"{car} with such data --> {data}")
