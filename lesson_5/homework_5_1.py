...
def filter_cars(car_data, search_criteria):
    ...
    for name, details in filtered_cars[:5]:
        print(f"{name}: Color={details[0]}, Year={details[1]}, Engine={details[2]}, Type={details[3]}, Price=${details[4]}")

# Execute function
filter_cars(car_data, search_criteria)
...
people_records = [
  ('Yaroslav', 'Bykov', 24, 'Software Engineer', 'Poznan'),  # New record
...
# Swap elements at index 1 and 5
...
# Check if people at indexes 6, 10, 13 have age >= 30
indexes_to_check = [6, 10, 13]
age_check = all(people_records[i][2] >= 30 for i in indexes_to_check)
if age_check == True:
    print("All selected people have age >= 30")
else:
    print("Not all people are older than 30")