...
for car, details in car_data.items():
    ...
    if year >= search_criteria[0] and engine >= search_criteria[1] and price <= search_criteria[2]:
        matching_cars.append((car,year, engine, price))
matching_cars.sort(key=lambda x: x[-1])
print("First found elements:")
for car, year, engine, price in matching_cars[:5]:
    print(f'{car}: {year} {engine} ${price} ')