...
for key, values in car_data.items():
    year = values[1]
    engine_volume = values[2]
    price = values[4]
    if year >= 2017:
        search_criteria[key] = values
    if engine_volume >= 1.6:
        search_criteria[key] = values
    if price <= 36000:
        search_criteria[key] = values

for key, values in list(search_criteria.items())[:5]:
    print(f"{key}: Рік - {values[1]}, Об'єм двигуна - {values[2]} л, Ціна - {values[4]}$")